...
dockerenv="{
	\"proxies\": {
   		\"default\": {
				...
				\"httpsProxy\": \"http://${CORRAL_registry_private_ip}:3219\",
     			\"noProxy\": \"localhost,127.0.0.1,127.0.0.0/8,0.0.0.0,10.0.0.0/8,cattle-system.svc,192.168.10.0/24,.svc,.cluster.local\"
   }
 }
}"
...
#proxy settings for docker daemon as root
sudo mkdir -p /etc/systemd/system/docker.service.d
sudo touch /etc/systemd/system/docker.service.d/http-proxy.conf
dockerproxy="
[Service]
Environment="HTTPS_PROXY=http://${CORRAL_registry_private_ip}:3219"
Environment="HTTP_PROXY=http://${CORRAL_registry_private_ip}:3219""
